...
if __name__ == "__main__":
	...
	DATASET_NAME = 'etl_952_singlechar_size_64_all_0.1'
	print('----------------')
	...
	trainA_path = os.path.join(datafolder, 'trainB')
	trainB_path = os.path.join(datafolder, 'trainA')
	testA_path = os.path.join(datafolder, 'valB')
	testB_path = os.path.join(datafolder, 'valA')
	...
	for x in np.arange(len(arr)):
		...
		if x <= ntrainA:
			imsave(os.path.join(trainA_path, str(ord(ch))+ '_' + str(count) + '.png'), bitmap)
		else:
	...
	for x in np.arange(len(arr)):
	    ...
	    if x <= ntrainB:
	        imsave(os.path.join(trainB_path, str(ord(ch)) + '_' + str(count) + '.png'), source_img)
	    else:
